...
class TWSECrawler:
    ...
    def fetch_index_data(self):
        ...
        try:
            ...
            # 輸出DataFrame的列名和前幾行以進行調試
            print(f"表格列名: {df.columns.tolist()}")
            print(f"表格前5行: {df.head()}")
            
            # 檢查所需列是否存在
            required_columns = ['日期', '成交金額', '成交股數', '成交筆數', '發行量加權股價指數', '漲跌點數']
            ...
            if len(df) > 0:
                ...
                if self.collection:
                    ...
                    print(f"數據保存{'成功' if success else '失敗'}")
